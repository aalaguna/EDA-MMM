...
server <- function(input, output, session) {
  ...
  # Seleccionar variables (excepto Variable 1, que es obligatoria)
  get_selected_variables <- function(input_vars, dataset) {
    vars <- input_vars[input_vars != "None"]
    vars <- vars[vars %in% names(dataset)]
    return(vars)
  }
  
  # Calcular la suma de las variables seleccionadas
  calculate_sum_variable <- function(dataset, vars) {
    dataset %>%
      mutate(Sum_Variables = rowSums(select(., all_of(vars)), na.rm = TRUE))
  }
  
  # Aplicar transformación a una variable (o suma de variables)
  apply_transformations <- function(data, var_name, trans_type, alpha, beta, maxval, decay, lag) {
    data[[var_name]] <- apply_transformation(
      data[[var_name]],
      trans_type,
      alpha  = alpha,
      beta   = beta,
  ...
  output$variables_chart_multi <- renderPlot({
    req(rv$filtered_data, input$kpi_multi)
    
    # Selección de variables
    vars_to_select <- c(input$var1_multi)
    if (!is.null(input$var2_multi) && input$var2_multi != "None") {
      vars_to_select <- c(vars_to_select, input$var2_multi)
    }
    if (!is.null(input$var3_multi) && input$var3_multi != "None") {
      vars_to_select <- c(vars_to_select, input$var3_multi)
    }
    ...
    if (input$sum_all_vars == "true") {
      ...
      plot_data <- data_chart %>%
        select(Sum_Variables, KPI = !!sym(input$kpi_multi))
      
      # Crear gráfico
      ggplot(plot_data, aes(x = Sum_Variables, y = KPI)) +
        geom_point(color = "blue", size = 2, alpha = 0.7) +
        geom_smooth(method = "lm", color = "red", se = FALSE) +
        theme_minimal() +
        labs(
          title = "Variable Flighting (Sum of Variables vs KPI)",
          x = "Sum of Selected Variables",
          y = input$kpi_multi
        )
    } else {
      # Si no se suman, trabajar con cada variable
      plot_data <- data_chart %>%
        select(all_of(vars_to_select), KPI = !!sym(input$kpi_multi)) %>%
        pivot_longer(cols = all_of(vars_to_select), names_to = "Variable", values_to = "Value")
      
      ggplot(plot_data, aes(x = Value, y = KPI, color = Variable)) +
        geom_point(alpha = 0.7) +
        geom_smooth(method = "lm", se = FALSE) +
        theme_minimal() +
        labs(
          title = "Variable Flighting (Individual Variables vs KPI)",
          x = "Value",
          y = input$kpi_multi
        )
    }
  ...
  output$s_curve_multivariate_plot <- renderPlotly({
    ...
    # Selección de variables
    vars_to_sum <- c(input$var1_multi)
    if (!is.null(input$var2_multi) && input$var2_multi != "None") {
      vars_to_sum <- c(vars_to_sum, input$var2_multi)
    }
    if (!is.null(input$var3_multi) && input$var3_multi != "None") {
      vars_to_sum <- c(vars_to_sum, input$var3_multi)
    }
    if (!is.null(input$var4_multi) && input$var4_multi != "None") {
    ...
    # Calcular suma de variables seleccionadas
    data_chart <- rv$filtered_data %>%
      mutate(Sum_Variables = rowSums(select(., all_of(vars_to_sum)), na.rm = TRUE))
    
    # Preparar datos para la curva S
    df_scurve <- data_chart %>%
      mutate(value = Sum_Variables) %>%
      select(Period = if_else("Period" %in% names(data_chart), "Period", "periodo"),
             value) %>%
      filter(!is.na(Period))
    ...
    # Generar curva S y Flighting
    flighting_plot <- create_flighting_chart(
      data_chart  = df_scurve,
    ...
    s_curve_plot <- create_s_curve_chart(
      data_chart  = df_scurve,
      alpha       = input$alpha_multi,
      beta        = input$beta_multi,
      max_val_pct = input$maxval_multi,
    ...
    # Combinar ambos gráficos en un solo layout
    subplot(flighting_plot, s_curve_plot, nrows = 1, titleX = TRUE, titleY = TRUE) %>%
